...
import { bytesToHex, stateToMatrix } from "../lib/aes128-decrypt";
...
const DecryptRoundCard = ({ round }) => (
    <section className="h-full rounded-2xl border border-slate-200 bg-white p-6 shadow-sm shadow-slate-200/50 transition hover:shadow-md">
        <header className="flex flex-col gap-2 pb-4 md:flex-row md:items-center md:justify-between">
            <div>
                <h3 className="text-lg font-bold uppercase tracking-[0.3em] text-rose-600">
                    {round.round === 0 ? "Initial Round" : `Round ${round.round}`}
                </h3>
                <p className="text-sm text-slate-500">
                    {round.round === 0
                        ? "Key mixing with round key 10 (reverse order)"
                        : round.round === 10
                            ? "Final round omits InvMixColumns"
...
export default DecryptRoundCard;
